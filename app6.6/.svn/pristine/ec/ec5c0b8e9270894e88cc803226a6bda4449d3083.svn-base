...
summerready = function() {
	...
	//用款申请单
	var paramStrselectApplyListByConditionList4Mobile = "CO_CODE="+userInfo.coCode+"&BILL_TYPE=EXP_MEETING&ND="+curr+"&APP_ID="+userInfo.userId;
	$.ajax({
		type : "get",
		async : false,
...
function fnSavebill(){
	...
	var meet_place = {//会议地点
		...
		recNo: "0", 
		value: $('#meetingPlace').val()
	};
	infoList.push(meet_place);
	var meet_name = {//会议名称
		infoId: "METTING_NAME",
		belongTable: "EXPENSE_COMMON_INFO00", 
		recNo: "0", 
		value: $('#meetingName').val()
	};
	infoList.push(meet_name);
	var people_num = {//参会人数
		infoId: "PEOPLE_NUMS",
		belongTable: "EXPENSE_COMMON_INFO00", 
		recNo: "0", 
		value: $('#meetingPeopleNum').val()
	};
	infoList.push(people_num);
	var meet_days = {//会议天数
		infoId: "days",
		belongTable: "EXPENSE_COMMON_INFO00", 
		recNo: "0", 
		value: $('#meetingDays').val()
	};
	infoList.push(meet_days);
	...
	var rent_standard = $("#rent_standard").val();
	if(rent_standard == ""){
		rent_standard = 0;
	}
	var rent_money = $("#rent_money").val();
	...
	var rent_remark = $("#rent_remark").val();
	var expense_rent_info = {
		...
		"expenseId": "RENT",
		"money": rent_money, 
		"peopleNumber": rent_count, 
		"quantity": "", 
		"quantity2": "", 
		"recNo": "2", 
		"remark": rent_remark ,
		"selfMoney": "0", 
		"standard": rent_standard,
		"standard2": "0",
		"standard3": "0",
		"tabId": "00", 
		"tabName": "明细单据(1)", 
	...
	var print_standard = $("#print_standard").val();
	if(print_standard == ""){
		print_standard = 0;
	}
	var print_money = $("#print_money").val();
	...
	//其他费用
	var other_count = $("#other_count").val();
	if(other_count == ""){
		other_count = 0;
	}
...
function fnLoaddetailInfo(billData){
	...
	for(var i=0;i < infoList.length;i++){
		if(infoList[i].infoId == "MEETING_PLACE"){//会议地点
			$('#meetingPlace').val(infoList[i].value);
		}else if(infoList[i].infoId == "METTING_NAME"){//会议名称
			$('#meetingName').val(infoList[i].value);
		}else if(infoList[i].infoId == "PEOPLE_NUMS"){//参会人数
			$('#meetingPeopleNum').val(infoList[i].value);
		}else if(infoList[i].infoId == "days"){//会议天数
			$('#meetingDays').val(infoList[i].value);
	...
	for(var i=0;i < expenseList.length;i++){
		if(expenseList[i].expenseId == "ROOM"){//住宿费
			$('#room_count').val(expenseList[i].peopleNumber);
			...
			$('#meal_count').val(expenseList[i].peopleNumber);
			...
			$('#rent_count').val(expenseList[i].peopleNumber);
			$('#rent_standard').val(expenseList[i].standard);
			$('#rent_money').val(expenseList[i].money);
			$('#rent_checkMoney').val(expenseList[i].checkMoney);
			$('#rent_remark').val(expenseList[i].remark);
			...
			detail_sum[2] = detail_rent_info;
			summer.setStorage('detail_sum', JSON.stringify(detail_sum));
		}else if(expenseList[i].expenseId == "PRINT"){//资料印刷费
			$('#print_count').val(expenseList[i].peopleNumber);
			$('#print_standard').val(expenseList[i].standard);
			$('#print_money').val(expenseList[i].money);
			$('#print_checkMoney').val(expenseList[i].checkMoney);
			$('#print_remark').val(expenseList[i].remark);
			...
			$('#other_count').val(expenseList[i].peopleNumber);
