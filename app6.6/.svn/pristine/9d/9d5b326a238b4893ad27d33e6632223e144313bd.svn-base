...
function loadData(nd){
	...
	var urlParam="nd="+ nd;
	urlParam += "&INPUTOR_ID=" + userInfo.userId;
	urlParam += "&CLIENT=MOBILE";
	$.ajax({
		type : "POST",
		async : false,
		url : "http://" + userInfo.ip + "/FS/services/salaryService/selectFundList?" + urlParam,
		success : function(data) {
			...
			for (i in data.rows) {
				data.rows[i].ZCJZ=util.changeTwoDecimal(data.rows[i].ZCJZ);
			}
			rows=data.rows;
