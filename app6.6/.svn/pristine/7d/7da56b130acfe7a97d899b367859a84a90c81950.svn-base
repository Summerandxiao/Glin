var billId;//经费报销单billId
var billNo;//经费报销单billNo
var billStatus;
var isSave = 0;
var userInfo;
var ip;
...
summerready = function() {
    ...
    summer.rmStorage("info_list");
    summer.rmStorage("expense_List");
    summer.rmStorage("dest_detail_sum");
...
function fnLoaddetailInfo(billData){
	...
	for(var i=0;i<infoList.length;i++){
		if(infoList[i].infoId == "KZ_CONTEXT"){//开支内容
			kanr.push(infoList[i]);
		}else if(infoList[i].infoId == "ticketCount"){//票据张数
			$('#ticketCount').val(infoList[i].value) ;
		}
	}

	for(var i=0;i<kanr.length;i++){
		//开支内容
		var kz_content = {
			infoId: "KZ_CONTEXT",
			belongTable: "EXPENSE_INFO00",
			recNo: kanr[i].recNo,
			value: kanr[i].value // 开支内容
		};
		info_list.kz_context[`id`+ i] = kz_content;
	}
	...
	for(var i=0;i<expenseList.length;i++){
		var expense_detail = {
			expenseId: "NO_DEFINE",
		 	belongTable: "EXP_OUTLAY00", 
			recNo: expenseList[i].recNo, 
			money: expenseList[i].money, 
			currencyKind: "RMB", 
			exchangeRate: "1", 
			currencyMoney: expenseList[i].currencyMoney, 
			selfMoney: "0", 
			checkMoney: expenseList[i].checkMoney, // 核定金额
			ticketCount: "0", 
			remark: expenseList[i].remark, // 备注
			dependRemark: "", 
			budgetMoney: "", 
			standard: "0",  
			peopleNumber: "", 
		...
		expense_List[`id`+i]= expense_detail;

		sum_money.push(expenseList[i].money*1);
		var dest_detail_obj = {
			sum_money: expenseList[i].money*1, // 总金额
		};
		...
		$('#transportation .detail-btn').append(
			...
			`<li class="um-listgroup-row um-listview-row">
				<a href="#goDest" class="um-list-item main-item go-dest-cost go-next-page um-swipe-action">
					<div class="um-list-item-inner">
						<div class="um-list-item-body">
							<h5 class="um-media-heading um-blue">开支内容：`+kanr[i].value+`</h5>
						</div>
						<div class="um-list-item-right">
