...
function loadData() {
	...
	$.ajax({
		type : "get",
		async : false,
		url : "http://" + userInfo.ip + "/FS/services/fundQueryService/selectorgBudgetPayoutPeriod_Mobile?" + urlParam,
...
function subgo(obj) {
	...
	listLink.on("click", function(e) {//项目经费明细+凭证信息 穿透
		...
		if (parent) {
			...
			//保存点击的数据 在报销详情页取出
			summer.openWin({
				id : 'orgPayoutQueryDetail',
				url : 'html/orgPayoutQueryDetail.html',
				reload : true,
			});
			UM.hideLoadingBar();
		}
