...
summerready = function() {
	...
	if (summer.getStorage('userInfo_local')) {//有数据
		...
		//alert("userInfo_local:"+JSON.stringify(userInfo_local));
		//设置checked的状态
...
var userLogin = function() {
	...
	var paramStrOnePwd =  "&USER_NAME=" + encodeURIComponent(encodeURIComponent(user)) + "&IS_WEB=1";
	console.log(paramStrOnePwd)
	$.ajax({
			type : "POST",
			async : false,
			dataType : "json",
			url : "http://" + ip + "/FS/services/appVersionService/countLogUserNum?"+ paramStrOnePwd,
			success : function(data) {
				if (data.resultCode == 1) {
					var versionData = data.rows;
					if (versionData == 0) {
						/*
						summer.setStorage("uusername", encodeURIComponent(encodeURIComponent(user)));
						summer.setStorage("upassword", encodeURIComponent(password));
						uusername = summer.getStorage("uusername");
						upassword = summer.getStorage("upassword");
						*/
						var uusername = encodeURIComponent(encodeURIComponent(user));
						var upassword =	encodeURIComponent(password);
								...
								success : function(data) {
									if (data.resultCode == 1) {
										...
										if(c){
											...
											summer.setStorage("username", uusername);
											summer.setStorage("password", upassword);
											summer.setStorage("uid", versionData.ID);
											//not kong
											//password = upassword;
											//uid = versionData.ID;
											summer.openWin({
												id : 'oneUpdatePwd',
												url : 'html/oneUpdatePwd.html',
												reload : true,
												pageParam : {
													uid : 'uid',
													password : 'password',
						...
						/**var uid = userInfosrt.uid;
						var password = userInfosrt.userPass;
