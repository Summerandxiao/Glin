var billId;//差旅报销单billId
var billNo;//差旅报销单billNo
var billStatus;
var isSave = 0;
var userInfo;
var ip;
...
function fnLoaddetailInfo(billData){
	...
	var infoList = $.parseJSON(billData.infoList);//infoList
	var mx_count = 0;
	var ccrs = [];//出差人数
	var ccr = [];//出差人
	var cfrq = [];//出发日期
	...
	var dddd = [];//到达地点
	var zsdd = [];//市内交通费/住宿地点
	var city_jtfrs = [];//市内交通费人数
	var beizhu = [];//备注
	for(var i=0;i<infoList.length;i++){
		if(infoList[i].infoId == "ticketCount"){//票据张数
			$('#ticketCount').val(infoList[i].value) ;
		}else if(infoList[i].infoId == "PEOPLE_NUMBER"){//出差人数
			ccrs.push(infoList[i]);
		}else if(infoList[i].infoId == "MEMBERS"){//出差人
			ccr.push(infoList[i]);
		}else if(infoList[i].infoId == "START_DATE"){//出发日期
			cfrq.push(infoList[i]);
		}else if(infoList[i].infoId == "END_DATE"){//返回日期
			fhrq.push(infoList[i]);
		}else if(infoList[i].infoId == "DEPARTURE"){//出发地点
			cfdd.push(infoList[i]);
		}else if(infoList[i].infoId == "LOCATION"){//到达地点
			dddd.push(infoList[i]);
		}else if(infoList[i].infoId == "EXP_TRIP_EXTEND_01.ZS_CITY"){//市内交通费/住宿地点
			zsdd.push(infoList[i]);
		}else if(infoList[i].infoId == "EXP_TRIP_EXTEND_01.PEOPLE_NUMBER"){//市内交通费人数
			city_jtfrs.push(infoList[i]);
		}else if(infoList[i].infoId == "REMARK"){//备注
			beizhu.push(infoList[i]);
		}
	}
	mx_count = ccr.length;
	for(var i=0;i<ccrs.length;i++){		//出差人数
		var trip_person_num = {
			infoId: "PEOPLE_NUMBER",
			belongTable: "EXPENSE_INFO00",
			recNo: ccrs[i].recNo,
			value: ccrs[i].value // 出差人数
		};
		info_list.trip_person_num[`id`+ i] = trip_person_num;
	}
	for(var i=0;i<ccr.length;i++){
		...
		var trip_man = {
			...
			belongTable: "EXPENSE_INFO00",
			recNo: ccr[i].recNo,
			value: ccr[i].value // 出差人
		};
		info_list.trip_man[`id`+ i] = trip_man;
		clickIndex++;
	}
	for(var i=0;i<cfrq.length;i++){
		//出发日期
		var depart_time = {
			infoId: "START_DATE",
			belongTable: "EXPENSE_INFO00",
			recNo: cfrq[i].recNo,
			value: cfrq[i].value  // 出发日期
		};
		info_list.depart_time[`id`+ i] = depart_time;
	}
	for(var i=0;i<fhrq.length;i++){
		//返回日期
		var back_time = {
			infoId: "END_DATE",
			belongTable: "EXPENSE_INFO00",
			recNo: fhrq[i].recNo,
			value: fhrq[i].value // 返回日期
		};
		info_list.back_time[`id`+ i] = back_time;
	}
	for(var i=0;i<cfdd.length;i++){
		//出发地点
		var depart_spot = {
			infoId: "DEPARTURE",
			belongTable: "EXPENSE_INFO00",
			recNo: cfdd[i].recNo,
			value: cfdd[i].value // 出发地点
		};
		info_list.depart_spot[`id`+ i] = depart_spot;
	}
	for(var i=0;i<dddd.length;i++){
		...
		var arrive_spot = {
			...
			belongTable: "EXPENSE_INFO00",
			recNo: dddd[i].recNo,
			value: dddd[i].value // 到达地点
		};
		info_list.arrive_spot[`id`+ i] = arrive_spot;
	}	
	for(var i=0;i<zsdd.length;i++){
		//市内交通费/住宿地点
		var traffic_place = {
			infoId: "EXP_TRIP_EXTEND_01.ZS_CITY",
			belongTable: "EXPENSE_INFO00",
			recNo: zsdd[i].recNo,
			value: zsdd[i].value // 市内交通费/住宿地点
		};
		info_list.traffic_place[`id`+ i] = traffic_place;
	}
	for(var i=0;i<city_jtfrs.length;i++){
		//市内交通费人数
		var traffic_person = {
			infoId: "EXP_TRIP_EXTEND_01.PEOPLE_NUMBER",
			belongTable: "EXPENSE_INFO00",
			recNo: city_jtfrs[i].recNo,
			value: city_jtfrs[i].value// 市内交通费人数
		};
		info_list.traffic_person[`id`+ i] = traffic_person;
	}
	for(var i=0;i<beizhu.length;i++){
		...
		var remark = {
			infoId: "REMARK",
			belongTable: "EXPENSE_INFO00",
			recNo: beizhu[i].recNo,
			value: beizhu[i].value // 备注
		};
		info_list.remark[`id`+ i] = remark;
	}
	
	var expenseList = $.parseJSON(billData.expenseList);//expenseList
	var traffic = [];//城市间交通费金额
	var stay = [];//住宿费
	var city_traffic = [];//市内交通费
	var food = [];//伙食补助
	var other = [];//其他费用
	for(var i=0;i<expenseList.length;i++){
		if(expenseList[i].expenseId == "TRANSPORT"){
			traffic.push(expenseList[i]);
		}else if(expenseList[i].expenseId == "ROOM"){
			stay.push(expenseList[i]);
		}else if(expenseList[i].expenseId == "EXP_TRIP_EXTEND_01"){
			city_traffic.push(expenseList[i]);
		}else if(expenseList[i].expenseId == "MEAL_SUBSIDY"){
			food.push(expenseList[i]);
		}else if(expenseList[i].expenseId == "L_NO_STANDARD_other"){
			other.push(expenseList[i]);
		}
	}	
	...
	for(var i=0;i<traffic.length;i++){
		...
		var city_traffic_fare = {
			...
			isZero: false, 
			belongTable: "EXP_TRIP00", 
			recNo: traffic[i].recNo, 
			money: traffic[i].money, //城市间交通费金额
			currencyKind: "RMB", 
			exchangeRate: "1", 
			currencyMoney: traffic[i].currencyMoney, 
			selfMoney: "0", 
			checkMoney: traffic[i].checkMoney, 
			ticketCount: "", 
			remark: "",  
			dependRemark: "",  
			budgetMoney: "",  
		...
		expense_List.city_traffic_fare[`id`+i]= city_traffic_fare;//城市间交通费金额
	}
	for(var i=0;i<stay.length;i++){
		...
		var stay_fare = {
			...
			belongTable: "EXP_TRIP00", 
			recNo: stay[i].recNo, 
			money: stay[i].money, // 住宿费金额 
			currencyKind: "RMB", 
			exchangeRate: "1", 
			currencyMoney: stay[i].currencyMoney, // 住宿费金额 
			selfMoney: "0", 
			checkMoney: stay[i].checkMoney, // 住宿费金额 
			ticketCount: "", 
			remark: "",
			dependRemark: "", 
			budgetMoney: "",
			balanceMoney: "",
			standard: stay[i].standard, // 住宿费标准
			standard2: "",
			standard3: "",
			peopleNumber: "",
			days: stay[i].days, // 住宿费天数
			days2: "", 
			days3: "",
			peopleNumber2: "", 
			peopleNumber3: "",
		...
		expense_List.stay_fare[`id`+i]= stay_fare;//住宿费
	}
	for(var i=0;i<city_traffic.length;i++){
		//市内交通费	
		var traffic_fare = {
			expenseId: "EXP_TRIP_EXTEND_01", 
			expenseName: "",
			isZero: false, 
			belongTable: "EXP_TRIP00", 
			recNo: city_traffic[i].recNo,  
			money: city_traffic[i].money, // 市内交通费金额
			currencyKind: "RMB",
			exchangeRate: "1", 
			currencyMoney: city_traffic[i].currencyMoney, // 市内交通费金额
			selfMoney: "0", 
			checkMoney: city_traffic[i].checkMoney, // 市内交通费金额
			ticketCount: "",
			remark: "",
			dependRemark: "",
			budgetMoney: "",
			balanceMoney: "",
			standard: city_traffic[i].standard, // 市内交通费标准
			standard2: "",
			standard3: "",
			peopleNumber: "",
			days: city_traffic[i].days, // 市内交通费天数
			days2: "",
			days3: "",
			peopleNumber2: "", 
			peopleNumber3: "",
		...
		expense_List.traffic_fare[`id`+i]= traffic_fare;//市内交通费
	}
	for(var i=0;i<food.length;i++){
		...
		var food_fare = {
			...
			belongTable: "EXP_TRIP00", 
			recNo: food[i].recNo, 
			money: food[i].money, // 伙食补助金额
			currencyKind: "RMB",
			exchangeRate: "1", 
			currencyMoney: food[i].currencyMoney, // 伙食补助金额
			selfMoney: "0", 
			checkMoney: food[i].checkMoney, // 伙食补助金额
			ticketCount: "",
			remark: "",
			dependRemark: "",
			budgetMoney: "",
			balanceMoney: "",
			standard: food[i].standard, // 伙食补助标准
			standard2: "", 
			standard3: "", 
			peopleNumber: "",
			days: food[i].days, // 伙食补助天数
			days2: "",
			days3: "",
			peopleNumber2: "",
			peopleNumber3: "", 
		...
		expense_List.food_fare[`id`+i]= food_fare;//伙食补助
	}
	...
	for(var i=0;i<other.length;i++){
		...
		var other_fare = {
			...
			belongTable: "EXP_TRIP00", 
			recNo: other[i].recNo, 
			money: other[i].money, // 其他费用金额
			currencyKind: "RMB",
			exchangeRate: "1", 
			currencyMoney: other[i].currencyMoney, // 其他费用金额 
			selfMoney: "0", 
			checkMoney: other[i].checkMoney, // 其他费用金额
			ticketCount: "",
			remark: "",
			dependRemark: "",
			budgetMoney: "",
		...
		expense_List.other_fare[`id`+i]= other_fare;//其他费用
	}
	var sum_money = [];
	for(var i=0;i< mx_count;i++){
		sum_money.push(traffic[i].money*1+stay[i].money*1+city_traffic[i].money*1+food[i].money*1+other[i].money*1);
		var dest_detail_obj = {
			sum_money: traffic[i].money*1+stay[i].money*1+city_traffic[i].money*1+food[i].money*1+other[i].money*1, // 总金额
		};
		dest_detail_sum[`id`+i] = dest_detail_obj;
	}
		...
		$('#transportation .detail-btn').append(
			...
			`<li class="um-listgroup-row um-listview-row">
				<a href="#goDest" class="um-list-item main-item go-dest-cost go-next-page um-swipe-action">
					<div class="um-list-item-inner">
						<div class="um-list-item-body">
							<h5 class="um-media-heading um-blue">出差人：`+ccr[i].value+`</h5>
						</div>
						<div class="um-list-item-right">
							<span class="um-gray mr15 money-count">`+sum_money[i]+`</span>
						</div>
					</div>
					<div class="um-swipe-btns">
		                <span class="um-swie-btn um-delete"><span class="ti-trash" > &nbsp;删除</span></span>
