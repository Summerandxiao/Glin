...
function fnLoaddetailInfo(billData){
	...
	for(var i=0;i < infoList.length;i++){
		...
		}if(infoList[i].infoId == "chehao"){//车号
			$('#carNumber').val(infoList[i].value);
		}
	}
	
	//报销明细
	for(var i=0;i < expenseList.length;i++){
		if(expenseList[i].expenseId == "OIL"){//燃料费
			$('#oil_money').val(expenseList[i].money);
			$('#oil_checkMoney').val(expenseList[i].checkMoney);
			$('#oil_remark').val(expenseList[i].remark);
			$('#oil_fee').text("金额："+expenseList[i].money);
			var detail_oil_info = {
				"money": expenseList[i].money
			}
			detail_sum[0] = detail_oil_info;
			summer.setStorage('detail_sum', JSON.stringify(detail_sum));
		}else if(expenseList[i].expenseId == "REPAIR"){//修理费
			$('#repair_money').val(expenseList[i].money);
			$('#repair_checkMoney').val(expenseList[i].checkMoney);
			$('#repair_remark').val(expenseList[i].remark);
			$('#repair_fee').text("金额："+expenseList[i].money);
			var detail_repair_info = {
				"money": expenseList[i].money
			}
			detail_sum[1] = detail_repair_info;
			summer.setStorage('detail_sum', JSON.stringify(detail_sum));
		}else if(expenseList[i].expenseId == "QIPEIFEI"){//汽配费
			$('#qipei_money').val(expenseList[i].money);
			$('#qipei_checkMoney').val(expenseList[i].checkMoney);
			$('#qipei_remark').val(expenseList[i].remark);
			$('#qipei_fee').text("金额："+expenseList[i].money);
			var detail_qipei_info = {
				"money": expenseList[i].money
			}
			detail_sum[2] = detail_qipei_info;
			summer.setStorage('detail_sum', JSON.stringify(detail_sum));
		}else if(expenseList[i].expenseId == "DAOQIAO"){//道桥
			$('#daoqiao_money').val(expenseList[i].money);
			$('#daoqiao_checkMoney').val(expenseList[i].checkMoney);
			$('#daoqiao_remark').val(expenseList[i].remark);
			$('#daoqiao_fee').text("金额："+expenseList[i].money);
			var detail_daoqiao_info = {
				"money": expenseList[i].money
			}
			detail_sum[3] = detail_daoqiao_info;
			summer.setStorage('detail_sum', JSON.stringify(detail_sum));
		}else if(expenseList[i].expenseId == "SAFE"){//保险费
			$('#safe_money').val(expenseList[i].money);
			$('#safe_checkMoney').val(expenseList[i].checkMoney);
			$('#safe_remark').val(expenseList[i].remark);
			$('#safe_fee').text("金额："+expenseList[i].money);
			var detail_safe_info = {
				"money": expenseList[i].money
			}
			detail_sum[4] = detail_safe_info;
